function [X, x_star, errors, norms, times] = metoda_gradient_accelerat(A, e, n, m, rata_de_invatare, max_iter, prag_gradient, momentum)
    ...
    % Initializare momentum la zero
    momentum_X = zeros(size(X));
    momentum_x_star = zeros(size(x_star));

    % Bucla pt. metoda gradient accelerata
    while iter < max_iter && norma_gradient > prag_gradient
        ...
        % Updatarea momentumului
        momentum_X = momentum * momentum_X - rata_de_invatare * dL_dX;
        momentum_x_star = momentum * momentum_x_star - rata_de_invatare * dL_dx_star;

        % Updatarea parametrilor
        X = X + momentum_X;
        x_star = x_star + momentum_x_star;
        ...
        % Afisare progres
        if mod(iter, 1000) == 0
            fprintf('Iteratia: %d: Eroare = %f, Norma Gradient = %f\n', iter, sum(error.^2) / (2 * N), norma_gradient);
        end
    end
end
